...
tc_reduction_regress = []
for i in range(0, len(mkl_speedup)):
    for j in range(0, len(mkl_speedup[i])):
        tc1 = compute_tc_speedup[i][j]
        for m in range(0, i):
            tc2 = compute_tc_speedup[m][j]
            tmp_feature = []
            tmp_feature.append(np.log10(tc1/tc2))
            tmp_feature.append(k[m])
            for each in mat_basic_features[j]:
                tmp_feature.append(each)
            tmp_feature[4] = k[i]
            tc_reduction_regress.append(tmp_feature)

feature_csv_writer("./dim_tc_regress.csv", tc_reduction_regress)

#split the csv file
#k-fold cross validation
train_data = open("./dim_tc_regress.csv", 'r')
data_reader = csv.reader(train_data)
data_writers = []
write_file_name = []

for i in range(0, 10):
    tmp_file = open("./dim_tc_regress_data_" + str(i) + ".csv", 'w', newline='')
...
for k_fold_seed in range(0,10):
    ...
    for i in range(0, 10):
        if (i != k_fold_seed):
            tmp_file_name = "./dim_tc_regress_data_" + str(i) + ".csv"
    ...
    df_test = pd.read_csv('./dim_tc_regress_data_' + str(k_fold_seed) + ".csv", header=None, sep=',')
...
gbm.save_model('gbdt_dim_tc_regression.txt')