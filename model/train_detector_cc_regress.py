...
for i in range(0, len(mkl_speedup)):
    for j in range(0, len(mkl_speedup[i])):
        compute = compute_cc_speedup[i][j]
        trans = overall_cc_speedup[i][j] - compute                      
        tmp_feature = []
        tmp_feature.append(np.log10(compute/trans))
        for each in mat_basic_features[j]:
            tmp_feature.append(each)
        tmp_feature[3] = k[i]
        detector_cc_regress.append(tmp_feature)

feature_csv_writer("./detecter_cc_regress.csv", detector_cc_regress)

#split the csv file
#k-fold cross validation
train_data = open("./detecter_cc_regress.csv", 'r')
data_reader = csv.reader(train_data)
data_writers = []
write_file_name = []

for i in range(0, 10):
    tmp_file = open("./detecter_cc_regress_data_" + str(i) + ".csv", 'w', newline='')
...
for k_fold_seed in range(0,10):
    ...
    for i in range(0, 10):
        if (i != k_fold_seed):
            tmp_file_name = "./detecter_cc_regress_data_" + str(i) + ".csv"
    ...
    df_test = pd.read_csv('./detecter_cc_regress_data_' + str(k_fold_seed) + ".csv", header=None, sep=',')
...
gbm.save_model('gbdt_detector_cc_regression.txt')