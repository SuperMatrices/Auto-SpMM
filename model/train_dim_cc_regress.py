...
cc_reduction_regress = []
for i in range(0, len(mkl_speedup)):
    for j in range(0, len(mkl_speedup[i])):
        compute1 = compute_cc_speedup[i][j]
        for m in range(0, i):
            compute2 = compute_cc_speedup[m][j]
            tmp_feature = []
            tmp_feature.append(np.log10(compute1/compute2))
            tmp_feature.append(k[m])
            for each in mat_basic_features[j]:
                tmp_feature.append(each)
            tmp_feature[4] = k[i]
            cc_reduction_regress.append(tmp_feature)

feature_csv_writer("./dim_cc_regress.csv", cc_reduction_regress)

#split the csv file
#k-fold cross validation
train_data = open("./dim_cc_regress.csv", 'r')
data_reader = csv.reader(train_data)
data_writers = []
write_file_name = []

for i in range(0, 10):
    tmp_file = open("./dim_cc_regress_data_" + str(i) + ".csv", 'w', newline='')
...
for k_fold_seed in range(0,10):
    ...
    for i in range(0, 10):
        if (i != k_fold_seed):
            tmp_file_name = "./dim_cc_regress_data_" + str(i) + ".csv"
    ...
    df_test = pd.read_csv('./dim_cc_regress_data_' + str(k_fold_seed) + ".csv", header=None, sep=',')
...
gbm.save_model('gbdt_dim_cc_regression.txt')