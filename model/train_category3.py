...
for i in range(0, len(mkl_speedup)):
    for j in range(0, len(mkl_speedup[i])):
        ...
        tmp_feature[3] = k[i]
        category3_features.append(tmp_feature)

feature_csv_writer("./category3_data.csv", category3_features)

#split the csv file
#k-fold cross validation
train_data = open("./category3_data.csv", 'r')
data_reader = csv.reader(train_data)
data_writers = []
write_file_name = []

for i in range(0, 10):
    #tmp_file = open("../../test/model_test/lgb_train_data_" + str(i) + ".csv", 'w', newline='')
    tmp_file = open("./category3_data_" + str(i) + ".csv", 'w', newline='')
...
for k_fold_seed in range(0,10):
    ...
    for i in range(0, 10):
        if (i != k_fold_seed):
            tmp_file_name = "./category3_data_" + str(i) + ".csv"
    ...
    df_test = pd.read_csv('./category3_data_' + str(k_fold_seed) + ".csv", header=None, sep=',')
...
gbm.save_model('saved_category3.txt')