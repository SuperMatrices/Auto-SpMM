...
for i in range(0, len(mkl_speedup)):
    for j in range(0, len(mkl_speedup[i])):
        ...
        tmp_feature[3] = k[i]
        detector_tc_category.append(tmp_feature)

feature_csv_writer("./detecter_tc_category.csv", detector_tc_category)

#split the csv file
#k-fold cross validation
train_data = open("./detecter_tc_category.csv", 'r')
data_reader = csv.reader(train_data)
data_writers = []
write_file_name = []

for i in range(0, 10):
    tmp_file = open("./detecter_tc_category_data_" + str(i) + ".csv", 'w', newline='')
...
for k_fold_seed in range(0,10):
    ...
    for i in range(0, 10):
        if (i != k_fold_seed):
            tmp_file_name = "./detecter_tc_category_data_" + str(i) + ".csv"
    ...
    df_test = pd.read_csv('./detecter_tc_category_data_' + str(k_fold_seed) + ".csv", header=None, sep=',')
...
gbm.save_model('gbdt_detector_tc_category.txt')